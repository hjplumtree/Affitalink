...
export default function Navigator({ className, style }) {
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 z-[100] h-full border-r border-white/10 bg-[#12131a] px-3 pb-4 pt-4 backdrop-blur-xl transition-all",
        burgerMenuOpen ? "w-[220px]" : "w-[62px]",
        "lg:w-[220px]",
        className
      )}
      style={style}
    >
      <div className="relative mb-5 flex w-full items-center">
        <div className="flex items-center gap-3">
          ...
          <div className={cn("hidden lg:block", burgerMenuOpen && "block")}>
            ...
            <p className="text-xs text-white/75">Affiliate offer publishing system</p>
          </div>
        </div>

        <button
          type="button"
          className="absolute -right-[17px] grid h-7 w-7 place-items-center rounded-full border border-white/10 bg-[rgba(21,25,38,0.98)] text-white shadow-[0_10px_24px_rgba(0,0,0,0.25)] lg:hidden"
      ...
      <div className="w-full">
        <p className={cn("mb-4 px-3 text-xs leading-5 text-white/80", !burgerMenuOpen && "hidden", "lg:block")}>
          Collect offers from affiliate networks, review the changes, and turn them into publishable inventory for your real site.
        </p>
        <div className={cn("mx-3 mb-4 rounded-[18px] border border-white/10 bg-white/5 p-3", !burgerMenuOpen && "hidden", "lg:block")}>
          <p className="text-[10px] uppercase tracking-[0.16em] text-white/60">V1</p>
          <p className="mt-1 text-sm font-bold text-white">
            Canonical offer library and review workflow for publishers.
          </p>
        </div>
        <div className="mb-4 h-px bg-white/10" />

        <RouterLink to="/" className={cn(!burgerMenuOpen && "justify-center lg:justify-start", "text-white")}>
          <FaHome className="h-[18px] w-[18px]" />
          <span className={cn(!burgerMenuOpen && "hidden", "lg:inline")}>Home</span>
        </RouterLink>
        ...
        <RouterLink to="/offers" className={cn(!burgerMenuOpen && "justify-center lg:justify-start", "text-white")}>
          ...
          <span className={cn(!burgerMenuOpen && "hidden", "lg:inline")}>Library</span>
        </RouterLink>
        ...
        {user ? (
          <>
            ...
            <RouterLink to="/studio/offers" className={cn("mt-3 text-white", !burgerMenuOpen && "justify-center lg:justify-start")}>
              <FaTags className="h-[18px] w-[18px]" />
              <span className={cn(!burgerMenuOpen && "hidden", "lg:inline")}>Library studio</span>
            </RouterLink>
            <RouterLink to="/links" className={cn("mt-3 text-white", !burgerMenuOpen && "justify-center lg:justify-start")}>
              ...
              <span className={cn(!burgerMenuOpen && "hidden", "lg:inline")}>Review</span>
            </RouterLink>
          </>
        ) : null}
        ...
        <div className="mt-4 rounded-[20px] border border-white/10 bg-white/[0.03] p-2">
          {user ? (
            <button
              type="button"
              onClick={signOut}
