const {
  ...
  updateSnapshotPublishStatusById,
...
async function getOffers({ network, status = "active", publishStatus, workspaceId } = {}) {
  const scopedWorkspaceId = getScopeWorkspaceId(workspaceId);
...
async function updateOfferPublishStatus(id, action, options = {}) {
  const scopedWorkspaceId = getScopeWorkspaceId(options.workspaceId);
  const now = new Date().toISOString();
  const publishStatus =
    action === "select" ? "selected" : action === "publish" ? "published" : action === "unpublish" ? "draft" : null;

  if (!publishStatus) {
    throw new ConnectorError("validation", "Unsupported publish action");
  }

  const snapshot = await updateSnapshotPublishStatusById(
    id,
    scopedWorkspaceId,
    publishStatus,
  ...
  if (!snapshot) {
    throw new ConnectorError("validation", "Offer not found");
  }
...
async function updateReviewItemStatus(id, action, options = {}) {
  ...
  if (action === "approve" && updated.afterSnapshot?.id) {
    await updateSnapshotPublishStatusById(
      updated.afterSnapshot.id,
      getScopeWorkspaceId(options.workspaceId),
      updated.afterSnapshot.publishStatus === "published" ? "published" : "selected",
      updated.afterSnapshot.publishStatus === "published" ? updated.afterSnapshot.publishedAt || new Date().toISOString() : null,
      new Date().toISOString()
    );
  }
