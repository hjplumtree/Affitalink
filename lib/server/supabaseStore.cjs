...
function snapshotToRow(snapshot) {
  return {
    ...
    publish_status: snapshot.publishStatus || "draft",
...
async function listSnapshots({ workspaceId, connectorId, network, status, publishStatus } = {}) {
  const supabase = getClient();
...
async function updateSnapshotPublishStatusById(id, workspaceId, publishStatus, publishedAt, updatedAt) {
  const supabase = getClient();
  let query = supabase
    .from("coupon_snapshots")
    .update({
  ...
  assertNoError(result, "Could not update snapshot publish status");
  return result.data ? mapSnapshotRow(result.data) : null;
}
...
async function insertSyncRun(syncRun) {
  ...
  const result = await supabase
    .from("sync_runs")
    .upsert(syncRunToRow(syncRun), { onConflict: "id" })
    .select()
    .maybeSingle();
  assertNoError(result, "Could not save sync run");
