...
async function fetchCJLinks(auth, ids) {
  ...
  try {
    const response = await fetch(
      `/offers?website-id=${auth.website_id}&advertiser-ids=${advertiser_ids}&records-per-page=50`,
      {
        headers: {
    ...
    if (!response.ok) {
      return "[CJ] Server error, try little later";
    }
    ...
    return "[CJ] Check the entered informtation again";
  }
}
