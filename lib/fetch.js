...
async function fetchCJAdvertisers(info) {
  ...
  try {
    ...
    return { page, advertisers: formatted_advertisers };
  } catch (e) {
    return `Please check the information you filled ${e.message}`;
    throw new Error("Fetching error : " + e.message);
  }
}

async function fetchRakutenAdvertisers(info) {
  return { page: 1, advertisers: [{ id: "fake id", name: "fake name" }] };
}
