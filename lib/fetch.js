...
async function fetchCJLinks(auth, ids) {
  ...
  try {
    ...
    if (!response.ok) {
      return "Provided information looks inaccurate, Please check again";
    }
...
async function fetchRakutenLinks({ client_id, client_secret }, ids) {
  ...
  try {
    const response = await fetch(
      `/rakuten-links?mid=${mid}&resultsperpage=100`,
      {
        headers: {
    ...
    if (!response.ok) {
      throw "Provided information looks inaccurate, Please check again";
    }
    let offers = [];
    const xml = await response.text();
    parseString(xml, (e, result) => {
...
async function fetchCJAdvertisers(auth) {
  ...
  try {
    ...
    if (!response.every((res) => res.ok)) {
      return "Provided information looks inaccurate";
    }
...
async function fetchRakutenAdvertisers(info) {
  ...
  try {
    const response = await fetch("/rakuten-advertisers", {
      headers: {
        Authorization: "Bearer " + token,
      },
    });
